...
let innerLimit = 5;
let outerLimit = 15;
let image_src = "texture/texture_img.jpg";
let video_src = "texture/texture_vid.mp4";
let PointLocationU = 90.0;
let PointLocationV = 1.0;
let ScaleValue = 1;
let copyVideo = false;
...
function Model(name) {
    ...
    this.BufferData = function (vertices, normals, texCoords) {
        ...
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([1, 0, 0]), gl.DYNAMIC_DRAW);
    ...
    this.Draw = function () {
        ...
        let Dxyz = CalcXYZ(PointLocationU, PointLocationV);
        gl.uniform3fv(shProgram.iScalePointWorldLocation, [Dxyz[0], Dxyz[1], Dxyz[2]]);
        gl.uniform2fv(shProgram.iScalePointLocation, [Dxyz[0], Dxyz[1]]);
...
function draw() {
    ...
    let rotateToPointZero = m4.axisRotation([-0.9, -0.5, 0.5], 1.5);
...
window.addEventListener("keydown", function (event) {
    switch (event.key) {
        ...
        case "ArrowUp":
            ...
            break;
        case "w":
            if (PointLocationV < 180) {
                PointLocationV += PointStep;
            }
            draw();
            break;
        case "s":
            ...
            break;
        case "a":
            if (PointLocationU < 90) {
                PointLocationU += PointStep;
            }
            draw();
            break;
        case "d":
        ...
        case "+":
            ...
            draw();
            document.getElementById("ScaleValue").value = ScaleValue.toFixed(2);
            document.getElementById("ScaleValue_text").innerHTML = ScaleValue.toFixed(2);
            break;
        case "-":
            if (ScaleValue > 0.5) {
                ScaleValue -= 0.1;
